...
func saveUserSettings(writer http.ResponseWriter, request *http.Request, params httprouter.Params) {
	timer := time.Now()
	logInfo("SETTINGS-USER", "Saving user settings started")
	email, _, _ := request.BasicAuth()
	var data UserDetailsDataInput
	err := json.NewDecoder(request.Body).Decode(&data)
	if err != nil {
		logError("SETTINGS-USER", "Error parsing data: "+err.Error())
		var responseData TableOutput
		responseData.Result = "nok: " + err.Error()
		writer.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(writer).Encode(responseData)
		logInfo("SETTINGS-USER", "Saving user settings ended")
		return
	}
	...
	defer sqlDB.Close()
	if err != nil {
		logError("SETTINGS-USER", "Problem opening database: "+err.Error())
		var responseData TableOutput
		responseData.Result = "nok: " + err.Error()
		writer.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(writer).Encode(responseData)
	...
	db.Save(&user)
	cacheUsers(db)
	logInfo("SETTINGS-USER", "User settings saved in "+time.Since(timer).String())
}

func loadUserSettings(writer http.ResponseWriter, email string) {
	timer := time.Now()
	logInfo("SETTINGS-USER", "Loading user settings")
	...
	if err != nil {
		...
		writer.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(writer).Encode(responseData)
		logInfo("SETTINGS-USER", "Loading user settings ended")
		return
	}
	var user database.User
	db.Where("id = ?", cachedUsersByEmail[email].ID).Find(&user)
	...
	logInfo("SETTINGS-USER", "User settings loaded in "+time.Since(timer).String())
}
